...
class StageBlock(torch.nn.Module):
    def __init__(self, kernelSize, opt):
        ...
        self.proj=torch.nn.Conv2d(in_channels=opt.channelNum,out_channels=opt.channelNum,kernel_size=kernelSize,bias=False)
    ...
    def forward(self,out_lastStage, degLF, originalLFSize):
        ...
        err1 = self.recon(self.proj(out_lastStage) - degLF) #[bxy,c,u,v]
...
class MainNet(torch.nn.Module):
    ...
    def forward(self, lf):
        ...
        degLF=self.proj_init(lf.permute(0,4,5,3,1,2).reshape(b*x*y,c,u,v))#[bxy,c,uDeg,vDeg]
        _,_,uDeg,vDeg = degLF.shape
        ...
        return degLF.reshape(b,x,y,c,uDeg*vDeg).permute(0,4,3,1,2), out.reshape(b,x,y,c,u,v).permute(0,4,5,3,1,2),  #[b,uDeg*vDeg,c,x,y] [b,u,v,c,x,y] 
