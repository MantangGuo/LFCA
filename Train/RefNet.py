...
class RefNet(torch.nn.Module):
    ...
    def forward(self,input):
        ...
        conv_ang_2=F.relu(self.conv_ang_2(conv_spa_2.reshape(b,u,v,64,x,y).permute(0,4,5,3,1,2).reshape(b*x*y,64,u,v))) #[64]

        conv_spa_3=F.relu(self.conv_spa_3(conv_ang_2.reshape(b,x,y,64,u,v).permute(0,4,5,3,1,2).reshape(b*u*v,64,x,y))) #[64]
        conv_ang_3=F.relu(self.conv_ang_3(conv_spa_3.reshape(b,u,v,64,x,y).permute(0,4,5,3,1,2).reshape(b*x*y,64,u,v))) #[64]

        conv_spa_4=F.relu(self.conv_spa_4(conv_ang_3.reshape(b,x,y,64,u,v).permute(0,4,5,3,1,2).reshape(b*u*v,64,x,y))) #[64]
        conv_ang_4=F.relu(self.conv_ang_4(conv_spa_4.reshape(b,u,v,64,x,y).permute(0,4,5,3,1,2).reshape(b*x*y,64,u,v))) #[64]

        conv_spa_5=F.relu(self.conv_spa_5(conv_ang_4.reshape(b,x,y,64,u,v).permute(0,4,5,3,1,2).reshape(b*u*v,64,x,y))) #[64]
        conv_ang_5=F.relu(self.conv_ang_5(conv_spa_5.reshape(b,u,v,64,x,y).permute(0,4,5,3,1,2).reshape(b*x*y,64,u,v))) #[64]

        conv_spa_6=F.relu(self.conv_spa_6(conv_ang_5.reshape(b,x,y,64,u,v).permute(0,4,5,3,1,2).reshape(b*u*v,64,x,y))) #[3]
        conv_ang_6=F.relu(self.conv_ang_6(conv_spa_6.reshape(b,u,v,64,x,y).permute(0,4,5,3,1,2).reshape(b*x*y,64,u,v))) #[3]
        
        conv_spa_7=F.relu(self.conv_spa_7(conv_ang_6.reshape(b,x,y,64,u,v).permute(0,4,5,3,1,2).reshape(b*u*v,64,x,y))) #[3]
        conv_ang_7=F.relu(self.conv_ang_7(conv_spa_7.reshape(b,u,v,64,x,y).permute(0,4,5,3,1,2).reshape(b*x*y,64,u,v))) #[3]
        
        conv_spa_8=F.relu(self.conv_spa_8(conv_ang_7.reshape(b,x,y,64,u,v).permute(0,4,5,3,1,2).reshape(b*u*v,64,x,y))) #[3]
        conv_ang_8=F.relu(self.conv_ang_8(conv_spa_8.reshape(b,u,v,64,x,y).permute(0,4,5,3,1,2).reshape(b*x*y,64,u,v))) #[3]
        
        conv_spa_9=F.relu(self.conv_spa_9(conv_ang_8.reshape(b,x,y,64,u,v).permute(0,4,5,3,1,2).reshape(b*u*v,64,x,y))) #[3]
        conv_ang_9=F.relu(self.conv_ang_9(conv_spa_9.reshape(b,u,v,c,x,y).permute(0,4,5,3,1,2).reshape(b*x*y,c,u,v))) #[3]

        output=conv_ang_9.reshape(b,x,y,c,u,v).permute(0,3,4,5,1,2) #[3]
